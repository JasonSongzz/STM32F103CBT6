/******************************************************************************
 * @file    I2C1.c
 * @brief   I2C1 hardware interface implementation (software bit-banging on 
 *          PA5, PA6)
 * @details This file implements the I2C1 hardware interface using either 
...
void I2C1_hw_init(void)
{
    ...
    /* Initialize lines to high level */
    I2C1_SCL_HIGH();
    I2C1_SDA_HIGH();
    I2C_FAST_DELAY;
}
...
/******************************************************************************
 * @brief   Generate I2C1 start condition
 * @details Generate start condition on I2C1 bus
 * @param   None
 * @return  None
 *****************************************************************************/
void I2C1_start(void)
{
    I2C1_SDA_HIGH();         /* SDA HIGH */
    I2C1_SCL_HIGH();         /* SCL HIGH */
    I2C_FAST_DELAY;
    I2C1_SDA_LOW();          /* SDA LOW  */
    I2C_FAST_DELAY;
    I2C1_SCL_LOW();          /* SCL LOW  */
    I2C_FAST_DELAY;
}

/******************************************************************************
 * @brief   Generate I2C1 stop condition
 * @details Generate stop condition on I2C1 bus
 * @param   None
 * @return  None
 *****************************************************************************/
void I2C1_stop(void)
...
/******************************************************************************
 * @brief   Send ACK signal on I2C1
 * @details Send acknowledge signal on I2C1 bus
 * @param   None
 * @return  None
 *****************************************************************************/
...
/******************************************************************************
 * @brief   Send NACK signal on I2C1
 * @details Send not-acknowledge signal on I2C1 bus
 * @param   None
 * @return  None
 *****************************************************************************/
...
/******************************************************************************
 * @brief   Wait for ACK signal on I2C1
 * @details Wait for acknowledge signal from I2C1 slave device
 * @param   None
 * @return  bool ACK status
 * @retval  true  ACK received
 * @retval  false NACK received or timeout
 *****************************************************************************/
...
/******************************************************************************
 * @brief   Send byte on I2C1 bus
 * @details Send one byte of data on I2C1 bus
...
/******************************************************************************
 * @brief   Read byte from I2C1 bus
 * @details Read one byte of data from I2C1 bus
...
/******************************************************************************
 * @brief   Initialize I2C1 hardware
 * @details Configure hardware I2C1 peripheral
 * @param   None
 * @return  None
 *****************************************************************************/
void I2C1_hw_init(void)
{
    ...
    RCC_APB1PeriphClockCmd(I2C1_PERIPH, ENABLE);
    
    /* Remap I2C1 pins if needed */
    GPIO_PinRemapConfig(I2C1_AF, ENABLE);
...
/******************************************************************************
 * @brief   Write data to I2C1 device using hardware I2C
...
/******************************************************************************
 * @brief   Read data from I2C1 device using hardware I2C
                           ...
                           uint16_t len)
{
    ...
    for (uint16_t i = 0; i < len; i++)
            data[i] = I2C1_read_byte(i == len - 1 ? 0 : 1);

    I2C1_stop();
    
    return I2C1_OK;
#else
    return I2C1_hw_read(addr, reg, data, len);
#endif
}
